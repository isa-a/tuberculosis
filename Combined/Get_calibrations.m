...
nsam = 1e3;
sams = prm.bounds(1,:) + diff(prm.bounds,1).*lhsdesign(nsam,size(prm.bounds,2));
...
mk = round(nsam/25);
for is = 1:nsam
    if mod(is,mk)==0; fprintf('%0.5g ', is/mk); end
    out(is) = obj(sams(is,:));
end
fprintf('\n');
...
for ii = 1:5
    ...
    ha
    [xsto, outsto] = MCMC_adaptive(obj, x0, 2e5, 1, [], [], [], 1);
end


for ii = 1:5
    x1 = fminsearch(nobj,x1,optimset('Display','iter'));
    x0 = x1;
end
