...
def goveqs_basis3(t, insert, i, s, M, agg, sel, r, p):
    ...
    # Handling index for auxiliary 'nTPT'
    nTPT_index = i['aux']['nTPT'][0] - 1  # subtract 1 for zero-based index

    # Ensure the index is within the correct range
    if nTPT_index >= 0 and nTPT_index < n_states:
        out[nTPT_index] = np.sum((sel['nTPT'] * M).dot(invec))
