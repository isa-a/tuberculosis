def allocate_parameters(x, p, r, prm, xi):
    ...
    r['gamma_2015'] = x[xi['gamma'][0]] * np.array([x[xi['p_relrate_gamma_chvad'][0]], 1])
    r['gamma_2020'] = x[xi['gamma'][1]] * np.array([x[xi['p_relrate_gamma_chvad'][0]], 1])

    r['progression'] = r['progression0'] * np.ones((2, 2, 3))
    mult = np.tile(np.array([[1, 0.4]]).reshape((1, 1, 2)), (2, 2, 1))
    r['progression'][:, :, 1:3] = r['progression0'] * x[np.array(xi['p_HIV_relrate']) - 1] * mult

    r['reactivation'] = r['reactivation0'] * np.ones((2, 2, 3))
    r['reactivation'][:, :, 1:3] = r['reactivation0'] * x[np.array(xi['p_HIV_relrate']) - 1] * mult
    ...
    for age_row in range(2):
        for age_col in range(2):
            for born_row in range(2):
                for born_col in range(2):
