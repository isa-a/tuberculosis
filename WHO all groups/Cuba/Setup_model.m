...
tmp(4,intersect(s.allI, [s.pos, s.art]))  = 1;
agg.inc = sparse(tmp);
...
for is1 = 1:length(set1)
    for is2 = 1:length(set2)
        for is3 = 1:length(set3)
            tmp(row, intersect(intersect(intersect(s.allI, set1{is1}), set2{is2}), set3{is3})) = 1;
            row = row+1;
        end
    end
end
...
f13  = get_distribution_fns(data.HIVincdpeak, 'lognorm', show);
f14  = get_distribution_fns(data.HIVincdnow, 'lognorm', show);
f15  = get_distribution_fns(data.incdTBHIV, 'lognorm', show);
f16  = get_distribution_fns(data.ARTcov, 'beta', show);
