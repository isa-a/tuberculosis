...
if cond1
    ...
    geq0a = @(t,in) goveqs_scaleup1D(t, in, M0, M4a, [1990:2010], i, s, p4a, r4a, prm, sel, agg);
...
%     [t1, soln1] = ode15s(geq1, [2010:2023], soln0a(end,:), options);
    ...
    geq1 = @(t,in) goveqs_scaleup2D(t, in, M0, M1, M2, M3, M4b, [2015 2023; 2010 2023; 2010 2023; 2010 2023], i, s, p4b, r4b, prm, sel, agg);
    [t1, soln1] = ode15s(geq1, [2010:2024], soln0a(end,:), options);
...
%   propincd_ch = dsol(end,i.aux.inc(4)) / dsol(end,i.aux.inc(1));
    ...
    incdTBHIV = dsol(end,i.aux.inc(4))*1e5;

    %coverage of ART
    ARTcov = sum(sfin(s.art))/sum(sfin([s.pos, s.art]));

    if incd > 0.1
        out  = calfn.fn(incd2010, incd2020, mort, p_vulnpopn, p_vulnTB, p_chpopn, ch_notifs, HIVincdpeak, HIVincdnow, incdTBHIV, ARTcov);
        aux.soln       = soln1;
        msg            = 2;
