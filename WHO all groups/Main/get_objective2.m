...
%     p_LTBI     = sum(sfin(intersect(s.migr_rect,[s.Lf, s.Ls])))/sum(sfin(s.migr_rect));
    ...
    if incd > 0.1
        out  = calfn.fn(incd2010, incdRR2020, mort, p_migrTB, p_migrpopn, p_LTBI, p_vulnpopn, p_vulnTB, incd_ch2020, p_chpopn, p_adpopn, ch_notifs);
        aux.soln       = soln1;
        msg            = 2;
        ...
        aux.sim        = [incd2010, incd2020, incdRR2020, mort, p_migrTB, p_migrpopn, p_LTBI, p_vulnpopn, p_vulnTB, p_chpopn, p_adpopn, ch_notifs];
    else
        out = -Inf;
        aux = NaN;
        msg = 1;
