...
%     p_LTBI     = sum(sfin(intersect(s.migr_rect,[s.Lf, s.Ls])))/sum(sfin(s.migr_rect));
    ...
    % Notifications
    notifs = dsol(end,i.aux.notifs)*1e5;
    
    if incd > 0.1
        out  = calfn.fn(incd2010, incd2020, mort, p_migrTB, p_migrpopn, p_LTBI, notifs);
        aux.soln       = soln1;
        msg            = 2;
        ...
        %aux.adpopn     = p_adpopn;
        aux.notifs  = notifs;
        aux.sim        = [incd2010, incd2020, mort, p_migrTB, p_migrpopn, p_LTBI, notifs];
    else
        out = -Inf;
        aux = NaN;
        msg = 1;
