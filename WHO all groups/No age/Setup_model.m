...
names = {'inc','incsources','mort','nTPT', 'notifs'};
lgths = [    2,          8,     1,     1,         1];
...
% --- Notifications
tmp = zeros(i.nstates);
tmp(intersect([s.Tx, s.Tx2],s.ad),:) = 1;
% Remove transitions due to changing migrant status
tmp(s.migr_rect, s.migr_long) = 0;
tmp(s.migr_long, s.migr_rect) = 0;
% Remove transitions due to changing drug resistance status
...
data.p_migrpopn     = [0.1 0.168 0.22];                                    % Proportion of population that is migrants
...
data.notifs         = [4000 4716 5200]/55.98e6*1e5;                           % notifications in the country   
...
f12  = get_distribution_fns(data.notifs, 'lognorm', show);
...
lhd.fn = @(incd2010, incd2020, mort, p_migrTB, p_migrpopn, p_LTBI, notifs) f1a(incd2010) + f1b(incd2020) + f2(mort) + f3(p_migrTB) + ...
