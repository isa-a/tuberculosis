...
%     vuln_relrisk = vuln_with_TB / no_vuln_with_TB;
    ...
    if incd > 0.1
        out  = calfn.fn(incd2010, incd2020, mort, p_vulnpopn, p_vulnTB, p_chpopn, ch_notifs);
        aux.soln       = soln1;
        msg            = 2;
...
%         aux.vuln_relrisk    = vuln_relrisk;
        aux.sim        = [incd2010, incd2020, mort, p_vulnpopn, p_vulnTB, propincd_ch, p_chpopn, p_adpopn, ch_notifs];
    else
        out = -Inf;
        aux = NaN;
        msg = 1;
