...
r.gamma_2015     = (x(xi.gamma(1))/1000)*[x(xi.p_relrate_gamma_chvad), 1];
r.gamma_2020     = x(xi.gamma(2))*[x(xi.p_relrate_gamma_chvad), 1];

tmp              = r.progression0*[1, x(xi.p_relrate(1)), 1, x(xi.p_relrate(2)), x(xi.p_relrate(3))];
r.progression    = [tmp*x(xi.p_relrate_factor); tmp];

tmp              = r.reactivation0*[1, x(xi.p_relrate(1)), 1, x(xi.p_relrate(2)), x(xi.p_relrate(3))];
...
r.vuln           = x(xi.r_vuln)/100;
p.relbeta_vuln   = 1;

r.ageing         = x(xi.ageing)/100;
