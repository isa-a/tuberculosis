function [out, aux, msg] = get_objective2(x, ref, prm, gps, calfn)
...
%     ch_notifs = dsol(end,i.aux.ch_notifs)*1e5;
    ...
    if incd(1) > 0.1
        % keyboard;
        out  = calfn.fn(incd2010, incd2020, mort, ART_covg, HIV_prev);
        aux.soln       = soln1;
        msg            = 2;
...
%         aux.ch_notifs  = ch_notifs;
        ...
        aux.sim        = [incd2010, incd2020, mort, ART_covg, HIV_prev];
    else
        % keyboard;
        out = -Inf;
        aux = NaN;
