...
%     M0b = make_model(p0b, r0b, i, s, gps, contmat);
    ...
    options = odeset('RelTol', 1e-1, 'AbsTol', 1e-3, 'MaxStep', 1, 'NonNegative', 1:i.nstates);
...
%     vuln_relrisk = vuln_with_TB / no_vuln_with_TB;
    ...
    if incd > 0.1
        out  = calfn.fn(incd2010, incd2020, mort, p_migrTB, p_LTBI_inmigr, p_vulnpopn, p_vulnTB, p_chpopn, ch_notifs);
        aux.soln       = soln1;
        msg            = 2;
