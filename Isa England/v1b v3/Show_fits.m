...
for ii = 1:size(xs,1)
    [out, aux] = obj(xs(ii,:));
    sims(ii,:) = [aux.incd2010, aux.incd2020, aux.mort, aux.p_migrTB, aux.p_migrpopn, aux.p_LTBI, aux.nTPT];
    inc(:,ii)  = aux.incd;
    pp(ii)     = aux.p_migrect;
end
...
% Collate data
alldat = [data.incd2010; data.incd2020; data.mort; data.p_migrTB; data.p_migrpopn; data.p_LTBI; data.nTPT2019];
den = alldat(:,2)';
